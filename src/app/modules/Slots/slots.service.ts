...
const getAllSlotsFromDB = async (query: Record<string, unknown>) => {
    ...
    const meta = await SlotsQuery.countTotal()
    return {
        meta,
        result
    };
...
const updateSlotsIntoDB = async (id: string) => {
    const result = await Slots.findByIdAndUpdate(id, {
        isDeleted: false
    }, {
        new: true
    })
...
const deleteSlotsIntoDB = async (id: string) => {
